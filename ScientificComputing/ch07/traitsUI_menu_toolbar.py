...
演示菜单、工具栏和状态栏的制作方法
"""
from traits.api import HasTraits, Code, Str, Int, on_trait_change
...
class MenuDemo(HasTraits):
    ...
    def traits_view(self):
        ...
        tool_bar = ToolBar(
            Action(
                image=ImageResource("folder_page.png", search_path=["ch07/img"]),
                tooltip=u"打开文档",
                action="open_file"
            ),
            Action(
                image=ImageResource("disk.png", search_path=["ch07/img"]),
                tooltip=u"保存文档",
                action="save_file"
            ),
        )
