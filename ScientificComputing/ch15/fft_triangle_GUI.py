...
class TriangleWave(HasTraits):
    ...
    view = View(
        HSplit(
            VSplit(
                ...
                Item("peak_list", style="custom",
                     show_label=False, width=100, height=250)
            ),
            VGroup(
                Item("container", editor=ComponentEditor(
                    size=(600, 300)), show_label=False),
    ...
    def update_plot(self):
        ...
        # 计算各个频率的振幅
        fft_data = np.clip(20*np.log10(np.abs(fft_parameters))
                           [:self.fftsize/2+1], -120, 120)
