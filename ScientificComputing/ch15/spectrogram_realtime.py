...
from enableapi import Window, Component, ComponentEditor
...
class Demo(HasTraits):
    ...
    def _create_plot_component(self):
        ...
        self.data["frequency"] = np.linspace(
            0., SAMPLING_RATE/2.0, num=NUM_SAMPLES/2)
        for i in xrange(NUM_LINES):
            self.data['amplitude%d' % i] = np.zeros(NUM_SAMPLES/2)
        self.data["time"] = np.linspace(
            0., float(NUM_SAMPLES)/SAMPLING_RATE, num=NUM_SAMPLES)
        ...
        spectrogram_time = (0.0, SPECTROGRAM_LENGTH *
                            NUM_SAMPLES/float(SAMPLING_RATE))
    ...
    def on_timer(self, *args):
        ...
        if self.buffer_size > 1:
            ...
            if len(self.spectrum_list) > NUM_LINES:
                del self.spectrum_list[0]
    ...
    def get_audio_data(self):
        ...
        while not self.finish_event.is_set():
            audio_data = np.fromstring(
                stream.read(NUM_SAMPLES), dtype=np.short)
