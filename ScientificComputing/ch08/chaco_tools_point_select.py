...
from traitsui.api import View, Item, HSplit, VGroup, Heading
...
class PointSelectionDemo(HasTraits):
    ...
    def __init__(self, **traits):
        ...
        self.scatter = scatter = plot.plot(
            ("x", "y"), type="scatter", marker_size=4)[0]

        self.select_tools = {}
        print(self.select_tools)
        ...
        scatter.index.on_trait_change(
            self.selection_changed, 'metadata_changed')
        self.plot = plot
        self.data = data
    ...
    def selection_changed(self):
        ...
        self.green_selection = ["%d, (%f, %f)" % (
            s, x[s], y[s]) for s in selection]
        selection = metadata.get("selections_red", [])
        self.red_selection = ["%d, (%f, %f)" % (s, x[s], y[s])
                              for s in selection]
