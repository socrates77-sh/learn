# -*- coding: utf-8 -*-
import math
from traits.api import (Float, HasTraits, List, Array, on_trait_change,
                        Instance, Range, Button)
from traitsui.api import (View, TableEditor, Item, Group, HGroup,
                          VGroup, HSplit, ScrubberEditor, EnumEditor)
...
class Equalizer(HasTraits):
    ...
    def design_parameter(self):
        '''设计系数并计算频率响应'''
        try:
            self.b, self.a = design_equalizer(
                self.freq, self.Q, self.gain, SAMPLING_RATE)
        except:
...
class Equalizers(HasTraits):
    ...
    def recalculate_h(self):
        '''计算多组均衡器级联时的频率响应'''
        try:
            tmp = np.array([eq.h for eq in self.eqs if eq.h !=
                            None and len(eq.h) == len(W)])
