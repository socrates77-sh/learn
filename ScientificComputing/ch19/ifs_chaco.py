...
from traits.api import (HasTraits, Instance, Enum, List,
                        Int, Event, Bool, Str, Button)
from traitsui.api import Item, View, HGroup, EnumEditor
...
class TrianglesTool(AbstractOverlay):
    ...
    def normal_right_down(self, event):
        if len(self.points) == 0:
            return
        dist = np.array([(event.x-x)**2+(event.y-y)
                         ** 2 for x, y in self.points])
    ...
    def overlay(self, component, gc, view_bounds=None, mode="normal"):
        ...
        for i in range(0, count, 3):
            ...
            gc.draw_path()
        gc.restore_state()
        render_markers(gc, self.points[0::3], "circle",
                       4, (1, 0, 0, 0.5), 1, (1, 0, 0, 0.5))
        render_markers(gc, self.points[1::3], "circle",
                       4, (0, 1, 0, 0.5), 1, (0, 1, 0, 0.5))
        render_markers(gc, self.points[2::3], "circle",
                       4, (0, 0, 1, 0.5), 1, (0, 0, 1, 0.5))
